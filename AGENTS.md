...
- **`swift/`** — `LiveKitWakeWord` Swift package: native Core ML pipeline for iOS 16+ / macOS 13+. `WakeWordModel` (stateless predict) + `WakeWordListener` (actor around `AVAudioEngine`). Bundled `.mlpackage` frontends compiled to `.mlmodelc` at first use via `ResourceLoader`.
- **`examples/ios_wakeword/`** — SwiftUI demo app (iOS + macOS) that consumes the `swift/` package via a local SPM dependency (`path: ../../swift`). `WakewordEngine` wraps `AVAudioEngine` + a 2 s Int16 ring buffer + background `WakeWordModel.predict()`; `ContentView` renders score/volume graphs and an ANE/GPU/CPU compute-unit picker. Generated from `project.yml` via `xcodegen`.
