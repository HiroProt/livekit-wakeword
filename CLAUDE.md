...
```
Raw audio (16kHz) → [LiveKit APM] → [Silero VAD gate] → MelSpectrogramFrontend (ONNX) → SpeechEmbedding (ONNX) → Classifier (PyTorch) → [0,1]
                     NS/HPF/AGC     optional               n_fft=512, hop=160, n_mels=32  76×32×1 → 96-dim         16×96 → 1 score
```
...
- **`inference/`**
  - `model.py` — `WakeWordModel` class for simple prediction API (optional Silero VAD gate)
  - `listener.py` — `WakeWordListener` class for async microphone detection (LiveKit APM noise suppression)
  - `vad.py` — `SileroVAD` wrapper for speech detection

### Key Design Decisions

- **Feature extraction is numpy-based** (ONNX runtime), not torch tensors. Three bundled ONNX models (`melspectrogram.onnx`, `embedding_model.onnx`, `silero_vad.onnx`) are loaded via `importlib.resources` (see `resources/__init__.py`).
